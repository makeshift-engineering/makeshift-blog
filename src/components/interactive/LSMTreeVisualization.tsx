...
export default function LSMTreeVisualization() {
  ...
  const [log, setLog] = useState<string[]>(['Ready. Click "Write" to add entries.']);
  ...
  const handleWrite = useCallback(() => {
    const keys = ['user_id', 'email', 'name', 'age', 'city', 'role', 'status', 'score'];
    ...
    setMemtable((prev) => {
      ...
      if (newMemtable.length >= MEMTABLE_CAPACITY) {
        // Flush to SSTable
        setTimeout(() => {
          const sorted = [...newMemtable].sort((a, b) => a.key.localeCompare(b.key));
          const newSSTable: SSTable = { id: sstableId, entries: sorted, level: 0 };
  ...
  const handleCompact = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      addLog(`Compacted → SSTable L1-${sstableId} (${sortedEntries.length} entries, ${l0Tables.length} merged)`);
    }, 800);
  ...
  return (
    <div style={{ fontFamily: 'var(--font-mono)', fontSize: '0.8125rem' }}>
      ...
      <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1.25rem', flexWrap: 'wrap' }}>
        <button onClick={handleWrite} style={btnStyle(colors.accent, colors.accentSubtle)}>
          Write Entry
        </button>
        <button
          onClick={handleCompact}
          disabled={l0Count < 2 || isCompacting}
          style={btnStyle(colors.teal, colors.tealSubtle, l0Count < 2 || isCompacting)}
        >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
        ...
        <div style={sectionStyle}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem', minHeight: 60 }}>
            {walEntries.length === 0 && <span style={{ color: colors.fg3, fontSize: '0.75rem' }}>empty</span>}
            {walEntries.map((e) => (
              <div
                key={e.id}
                style={{
                  ...entryStyle,
                  borderColor: flashEntry === e.id ? colors.accent : colors.border,
                  boxShadow: flashEntry === e.id ? `0 0 8px ${colors.accentGlow}` : 'none',
        ...
        <div style={sectionStyle}>
          ...
          {/* Capacity bar */}
          <div style={{ background: colors.bg, borderRadius: 4, height: 4, marginBottom: '0.5rem', overflow: 'hidden' }}>
            <div
              style={{
                height: '100%',
                width: `${(memtable.length / MEMTABLE_CAPACITY) * 100}%`,
                background: memtable.length >= MEMTABLE_CAPACITY - 1 ? colors.amber : colors.accent,
                borderRadius: 4,
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem', minHeight: 60 }}>
            {memtable.length === 0 && <span style={{ color: colors.fg3, fontSize: '0.75rem' }}>empty</span>}
            {memtable.map((e) => (
              <div
                key={e.id}
                style={{
      ...
      <div style={{ ...sectionStyle, marginTop: '0.75rem' }}>
        ...
        {sstables.length === 0 && (
          <span style={{ color: colors.fg3, fontSize: '0.75rem' }}>No SSTables yet. Fill the memtable to trigger a flush.</span>
        )}
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
          {sstables.map((table) => (
            <div
              key={table.id}
              style={{
                background: table.level === 0 ? colors.tealSubtle : colors.purpleSubtle,
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.375rem' }}>
                <span style={{ color: table.level === 0 ? colors.teal : colors.purple, fontWeight: 500 }}>
                  L{table.level}-{table.id}
                </span>
                <span style={{ color: colors.fg3, fontSize: '0.6875rem' }}>{table.entries.length} entries (sorted)</span>
              </div>
              <div style={{ display: 'flex', gap: '0.375rem', flexWrap: 'wrap' }}>
                {table.entries.map((e, i) => (
                  <span key={i} style={{ color: colors.fg2, fontSize: '0.6875rem' }}>
                    {e.key}={e.value}
                  </span>
                ))}
              </div>
      ...
      >
        {log.map((msg, i) => (
          <div key={i} style={{ color: i === log.length - 1 ? colors.fg1 : colors.fg3, lineHeight: 1.6 }}>
            <span style={{ color: colors.fg3, marginRight: '0.5rem' }}>{String(i + 1).padStart(2, '0')}</span>
            {msg}
          </div>
        ))}
      </div>
...
function btnStyle(color: string, bg: string, disabled = false): React.CSSProperties {
  return {
